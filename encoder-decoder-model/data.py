...
# Required for import of src
current_dir = os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))
parent_dir = os.path.dirname(current_dir)
sys.path.insert(0, parent_dir)
from src.augmentImages import augment
...
class SALICON:
    ...
    def load_data(self, crossval=False, crossval_fold=0):
...
class PERSONALIZED:
    ...
    def __init__(self, data_path):
        self._target_size = config.DIMS["image_size_personalized"]

        self._dir_stimuli = data_path + "stimuli"
        self._dir_saliency = data_path + "saliency"
        self.data_path = data_path
        self.history_path = os.path.join(Path(Path(data_path).parent).parent, 'results', 'history')

        # Copy validation files from previous fold back to original directory
        if os.path.exists(self.data_path + "val-stimuli"):
            for file in _get_file_list(self.data_path + "val-stimuli"):
                copyfile(file, self._dir_stimuli + "/" + os.path.basename(file))

        if os.path.exists(self.data_path + "val-saliency"):
            for file in _get_file_list(self.data_path + "val-saliency"):
                copyfile(file, self._dir_saliency + "/" + os.path.basename(file))

        non_augmented_saliencies = [x for x in _find_files_in_dir(self._dir_saliency) if "_r_" not in x]
        non_augmented_stimuli = [x for x in _find_files_in_dir(self._dir_stimuli) if "_r_" not in x]

        non_augmented_saliencies, non_augmented_stimuli = _fix_consistency(non_augmented_saliencies, non_augmented_stimuli)
        total_non_augmented_saliencies = len(non_augmented_saliencies)
        total_non_augmented_stimuli = len(non_augmented_stimuli)

        total_non_augmented_images = 0
        if total_non_augmented_saliencies < total_non_augmented_stimuli:
            total_non_augmented_images = total_non_augmented_saliencies
        else:
            total_non_augmented_images = total_non_augmented_stimuli

        self.n_train = int(total_non_augmented_images * (1 - config.PARAMS["validation_split_size"]))  # Round down
        self.n_valid = total_non_augmented_images - self.n_train
        assert self.n_train + self.n_valid == total_non_augmented_images
    ...
    def load_data(self, crossval=False, crossval_fold=0):
        ...
        list_x = _get_file_list(self._dir_stimuli)
        list_y = _get_file_list(self._dir_saliency)
        list_x, list_y = _fix_consistency(list_x, list_y)
        _check_consistency(zip(list_x, list_y), len(list_x))
        indices = np.arange(self.n_train + self.n_valid)

        if crossval:

            # Clear history from previous training
            if os.path.exists(self.history_path):
                rmtree(self.history_path)
                os.makedirs(self.history_path)

            chunks = np.array_split(indices, config.PARAMS["cross-validation-folds"])
            valid_list_x = [list_x[idx] for idx in chunks[crossval_fold]]
            valid_list_y = [list_y[idx] for idx in chunks[crossval_fold]]
        else:
            excerpt = indices[self.n_train:]
            valid_list_x = [list_x[idx] for idx in excerpt]
            valid_list_y = [list_y[idx] for idx in excerpt]

        # Create a directory containing the validation files
        if os.path.exists(self.data_path + "val-stimuli"):
            rmtree(self.data_path + "val-stimuli")
        os.makedirs(self.data_path + "val-stimuli")

        if os.path.exists(self.data_path + "val-saliency"):
            rmtree(self.data_path + "val-saliency")
        os.makedirs(self.data_path + "val-saliency")

        for validation_file in valid_list_x:
            copyfile(validation_file, self.data_path + "val-stimuli/" + os.path.basename(validation_file))
            os.remove(validation_file)
        valid_list_x = [self.data_path + "val-stimuli/" + os.path.basename(validation_file) for validation_file in valid_list_x]

        for validation_file in valid_list_y:
            copyfile(validation_file, self.data_path + "val-saliency/" + os.path.basename(validation_file))
            os.remove(validation_file)
        valid_list_y = [self.data_path + "val-saliency/" + os.path.basename(validation_file) for validation_file in valid_list_y]

        valid_set = _fetch_dataset((valid_list_x, valid_list_y),
                                   self._target_size, False)

        print("Model will be validated on these images:")
        print([os.path.basename(file) for file in valid_list_x], "")

        # Training data augmentation
        if config.PARAMS["augment_data"]:
            print("Augmenting data...")
            augment(self._dir_stimuli)
            augment(self._dir_saliency)

        # Fetch training data
        train_list_x = _get_file_list(self._dir_stimuli)
        train_list_y = _get_file_list(self._dir_saliency)

        train_list_x, train_list_y = _fix_consistency(train_list_x, train_list_y)
        _check_consistency(zip(train_list_x, train_list_y), len(train_list_x))
        train_set = _fetch_dataset((train_list_x, train_list_y),
                                   self._target_size, True)
...
def get_dataset_iterator(phase, dataset, data_path, crossval=False, crossval_fold=0):
    ...
    if phase == "train":
        ...
        train_set, valid_set = dataset_class.load_data(crossval, crossval_fold)
...
def _get_file_list(data_path):
    ...
    data_list = list(sorted(data_list, key=lambda i: os.path.splitext(os.path.basename(i))[0]))
...
def _fix_consistency(filepaths_a, filepaths_b):
    """A consistency check that makes sure all files could successfully be
       found and stimuli names correspond to the ones of ground truth maps."""

    filenames_a = [os.path.splitext(os.path.basename(file))[0] for file in filepaths_a]
    filenames_b = [os.path.splitext(os.path.basename(file))[0] for file in filepaths_b]

    matching_files = list(set(filenames_a) & set(filenames_b))

    filtered_a = [path for path in filepaths_a if os.path.splitext(os.path.basename(path))[0] in matching_files]
    filtered_b = [path for path in filepaths_b if os.path.splitext(os.path.basename(path))[0] in matching_files]
...
def _find_files_in_dir(dirpath, filename=None):
    files = []
