...
then
  GT_PATH='data/cat2000/fixations'
  echo "Loading model named $MODEL_NAME""_""$OBSERVER..."
  encoder-decoder-model/load_model.sh "$MODEL_NAME""_""$OBSERVER"
else
...
rm -rf test-results/"$MODEL_NAME"/"$OBSERVER"/"$MODEL"-discrepancy
rm -rf test-results/"$MODEL_NAME"/"$OBSERVER"/"$MODEL"-saliency
...
sudo docker run -u root -v $(pwd):/labs -it --gpus all python3-tensorflow-dp python encoder-decoder-model/main.py test -d personalized -p data/cat2000/images
...
sudo docker run -u root -v $(pwd):/labs -it python3-tensorflow-dp python src/differentiate_maps.py -gt $GT_PATH -sal test-results/"$MODEL_NAME"/"$OBSERVER"/"$MODEL"-saliency -out test-results/"$MODEL_NAME"/"$OBSERVER"/"$MODEL"-discrepancy -orig data/cat2000/images
